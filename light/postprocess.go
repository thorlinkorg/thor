...
import (
	...
	"github.com/thorlinkorg/thor/common/bitutil"
...
func GetChtRoot(db thordb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func StoreChtRoot(db thordb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	diskdb, trieTable    thordb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db thordb.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
...
func GetBloomTrieRoot(db thordb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db thordb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db thordb.Database, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
