...
Clef can be used to sign transactions and data and is meant as a(n eventual) replacement for Thor's account management. This allows DApps to not depend on Thor's account management. When a DApp wants to sign data (or a transaction), it can send the content to Clef, which will then provide the user with context and asks for permission to sign the content. If the users grants the signing request, Clef will send the signature back to the DApp.
...
The general flow for signing a transaction using e.g. Thor is as follows:
![image](sign_flow.png)
...
* Thor todos
    ...
    - Relay: Thor should be started in `geth --signer localhost:8550`.
    - Currently, the Thor APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This type is 20 `bytes`, and is incapable of carrying checksum information. The signer uses `common.MixedcaseAddress`, which retains the original input.
    - The Thor API should switch to use the same type, and relay `to`-account verbatim to the external API.
* [x] Storage
    * [x] An encrypted key-value storage should be implemented.
    * See [rules.md](rules.md) for more info about this.
* Another potential thing to introduce is pairing.
  * To prevent spurious requests which users just accept, implement a way to "pair" the caller with the signer (external API).
  * Thus Thor/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
...
Clef listens to HTTP requests on `rpcaddr`:`rpcport` (or to IPC on `ipcpath`), with the same JSON-RPC standard as Thor. The messages are expected to be [JSON-RPC 2.0 standard](https://www.jsonrpc.org/specification).
