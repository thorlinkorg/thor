...
func newClient(ctx *cli.Context) *thorclient.Client {
	client, err := thorclient.Dial(ctx.GlobalString(nodeURLFlag.Name))
	if err != nil {
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, thorclient.NewClient(client))
	if err != nil {
