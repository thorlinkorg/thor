...
import (
	...
	"github.com/thorlinkorg/thor/accounts/external"
	"github.com/thorlinkorg/thor/accounts/keystore"
	"github.com/thorlinkorg/thor/accounts/scwallet"
	"github.com/thorlinkorg/thor/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of thor is "thor". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Thor", keep that.
	if name == "thor" || name == "thor-testnet" {
		name = "Thor"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "thor" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by thor 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'thor' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
