...
// It reproduces https://github.com/thorlinkorg/thor/issues/14943
...
func TestReward(t *testing.T) {
	...
	total := new(big.Int)
	re := initRewardInWei
	//totalReward := new(big.Int).Mul(totalRewardInEther, ether)
	//diff := totalReward
	fmt.Printf("==HEIGHT==\t========REWARD========\t===========TOTAL===========\n")
	for i := 1; re.Sign() > 0; i++ {
		height := big.NewInt(int64(i))
		re = calcReward(height)
		total.Add(total, re)
		if height.Uint64()%halvingPeriod.Uint64() == 0 {
			fmt.Printf("%10d\t%22d\t%28d\n", height, re, total)
		}
		//diff = new(big.Int).Sub(totalReward, total)
		//if diff.Cmp(big.NewInt(1)) < 0 {
		//	fmt.Printf("%8d\t%22d\t%28d\t%v\n", height, re, total, diff)
		//	break
		//}
	}
}
