...
var (
	...
	errInvalidSealResult = errors.New("submit work err")
)
...
func (s *remoteSealer) loop() {
	...
	for {
		...
		case result := <-s.submitWorkCh:
			...
			if s.submitWork(result.nonce, result.mixDigest, result.hash, result.sig, result.pub) {
				result.errc <- nil
			} else {
...
func (s *remoteSealer) submitWork(nonce types.BlockNonce, mixDigest common.Hash, sealhash common.Hash,
	signature *hexutil.Bytes, publicKey *hexutil.Bytes) bool {
	if s.currentBlock == nil {
	...
	if !s.noverify {
		...
		if signature == nil || len(*signature) < 70 { //70~72
			s.ethash.config.Log.Warn("Invalid signature", "signature", signature)
			return false
		}
		s.ethash.config.Log.Debug("submitWork", "header.Extra before copy sig", hexutil.Encode(header.Extra))
		copy(header.Extra[extraVanity:extraVanity+len(*signature)], *signature)
		if err := s.ethash.odinAccessor.VerifyOdinProof(header, *publicKey); err != nil {
			s.ethash.config.Log.Warn("Invalid odin proof", "sealhash", sealhash, "elapsed", common.PrettyDuration(time.Since(start)), "err", err)
			return false
		}
	}
