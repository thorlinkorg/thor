...
type Service struct {
	...
	eth    *thor.Ethereum     // Full Ethereum service if monitoring a full node
...
func New(url string, ethServ *thor.Ethereum, lesServ *les.LightEthereum) (*Service, error) {
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["eth"]; info != nil {
		network = fmt.Sprintf("%d", info.(*thor.NodeInfo).Network)
		protocol = fmt.Sprintf("eth/%d", thor.ProtocolVersions[0])
	} else {
