...
type ChainIndexer struct {
	chainDb  thordb.Database     // Chain database to index the data from
	indexDb  thordb.Database     // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb thordb.Database, indexDb thordb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
