...
func InitDatabaseFromFreezer(db thordb.Database) {
...
func iterateTransactions(db thordb.Database, from uint64, to uint64, reverse bool) (chan *blockTxHashes, chan struct{}) {
...
func IndexTransactions(db thordb.Database, from uint64, to uint64) {
...
func UnindexTransactions(db thordb.Database, from uint64, to uint64) {
	// short circuit for invalid range
	if from >= to {
		return
	}
