...
import (
	...
	"github.com/thorlinkorg/thor/common/mclock"
	"github.com/thorlinkorg/thor/common/prque"
	"github.com/thorlinkorg/thor/consensus"
	"github.com/thorlinkorg/thor/core/rawdb"
	"github.com/thorlinkorg/thor/core/state"
	"github.com/thorlinkorg/thor/core/state/snapshot"
	...
	"github.com/thorlinkorg/thor/trie"
)
...
type BlockChain struct {
	...
	db     thordb.Database // Low level persistent database to store final content in
...
// Processor.
func NewBlockChain(db thordb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	updateFn := func(db thordb.KeyValueWriter, header *types.Header) {
	...
	delFn := func(db thordb.KeyValueWriter, hash common.Hash, num uint64) {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			// Migrate all ancient blocks. This can happen if someone upgrades from Thor
			...
			//
			// But if the `TxIndexTail` is not nil, e.g. Thor is initialized with
	...
	writeLive := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			// tx indexes)
			if batch.ValueSize() >= thordb.IdealBatchSize {
				if err := batch.Write(); err != nil {
					return 0, err
				}
				size += batch.ValueSize()
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - thordb.IdealBatchSize)
			}
...
// sync, Thor will automatically construct the missing indices and delete
// the extra indices.
func (bc *BlockChain) maintainTxIndex(ancients uint64) {
	// Before starting the actual maintenance, we need to handle a special case,
	// where user might init Thor with an external ancient database. If so, we
	...
	indexBlocks := func(tail *uint64, head uint64, done chan struct{}) {
		...
		// If the user just upgraded Thor to a new version which supports transaction
